...
p_values = [pearsonr(X_train, y_train)[1] for col in X_train.columns]

# FDR correction
reject, pvals_fdr, _, _ = multipletests(p_values, method='fdr_bh') 

sig_features = [f for f,is_rejected in zip(X_train, reject) if is_rejected]
print(f"Significant features at 5% FDR: {sig_features}")
...
for i, f in enumerate(top3_features, start=1):
